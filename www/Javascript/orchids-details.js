...
Object.keys(CHARACTERISTICS).forEach((characteristic) => {
  ...
  orchidCharacteristicValue.textContent = data[characteristic].find(
    ({ id }) => id === orchidData[characteristic]
  ).description;
  ...
  if (DATA_RESTORE[characteristic]) {
    orchidCharacteristicValue.classList.add(
      `${characteristic}-${
        DATA_RESTORE[characteristic].find(
          ({ id }) => id === orchidData[characteristic]
        ).description
      }`
    );
